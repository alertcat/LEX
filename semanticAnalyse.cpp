...
#include "FunctionReturnExistedCheck.cpp"
...
extern _SymbolRecord* findSymbolRecord(_SymbolTable* currentSymbolTable, string id, int mode);//从符号表中找出id对应的记录

extern void inputFunctionCall(_FunctionCall *functionCallNode, string &functionCall, int mode);//获取函数调用
extern int inputExpression(_Expression *expressionNode, string &expression, int mode, bool isReferedActualPara);//获取表达式
extern void inputVariantRef(_VariantReference *variantRefNode, string &variantRef, int mode, bool isReferedActualPara);//获取变量引用
...
void createSubSymbolTableAndInit(){//创建子符号表 符号表定位
	...
	cout << "new currentSymbolTable: ";
	for(int i=0; i<currentSymbolTable->recordList.size(); i++){
		cout << currentSymbolTable->recordList[i]->id << " ";
	}
	cout << endl;
}
...
string SemanticAnalyseVariantReference(_VariantReference* variantReference){
	cout << "SemanticAnalyseVariantReference ";
	cout << variantReference->variantId.first << " " << variantReference->variantId.second;
	if(variantReference==NULL){
		cout << "[SemanticAnalyseVariantReference] pointer of _VariantReference is null" << endl;
		return "";
	}
	cout << "\tcurrentSymbolTable: ";
	for(int i=0; i<currentSymbolTable->recordList.size(); i++){
		cout << currentSymbolTable->recordList[i]->id << " ";
	}
	...
	cout << " record: " << record->id << " " << record->lineNumber << endl;
	//首先必须是函数
	//作为左值必须是当前函数名
...
void SemanticAnalyseVariant(_Variant* variant){
	...
	cout << "SemanticAnalyseVariant " << variant->type->type.first << " " << variant->variantId.first << " " << variant->variantId.second << endl;
